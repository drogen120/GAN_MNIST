...
flags = tf.app.flags
flags.DEFINE_integer("iter", 7000, "iter to train ")
        ...
        for i in range(iteration):
            ...
            if i %100 == 0 :
                ...
                if start:
                    ...
                    if sum_accuracy_test/10000.0 >= 0.9948:
                        break
...
def get_feature(batch_size ,id ):
    ...
    with tf.Graph().as_default():
        ...
        logits, end_points = mnist_net.net_infer(batch_images,is_training =False)
...
def main(_):
    ...
    start_R(FLAGS.C_iter,start= False)
    # start_GAN()
    while True:
        for i in range(10):
            get_feature(FLAGS.C_batch_size,i)
        start_GAN()
        start_R(FLAGS.C_iter)
